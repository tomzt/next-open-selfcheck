...
### 2.7 กฎ source-of-truth เดียวกันนี้ ใช้กับคู่ไฟล์ EN/TH อื่นด้วย

ไม่ใช่แค่ README — ไฟล์คู่ EN/TH ไหนก็ตามในโปรเจกต์นี้ยึดกฎเดียวกัน: **แก้ฉบับ
อังกฤษก่อนเสมอ แล้วจึง sync ไปฉบับไทย** แต่ละคู่ประกาศ source of truth ไว้ใน
quote block ต้นไฟล์ของตัวเอง (ดูตัวอย่างที่ `docs/future-considerations.md`)
คู่ไฟล์ที่มีอยู่ตอนนี้:
- `README.md` ↔ `README.th.md`
- `docs/future-considerations.md` ↔ `docs/future-considerations-th.md`
...
| **Batch scan → review → confirm** | ✅ | ✅ (Phase 3b, `BorrowScreen.tsx`/`ReturnScreen.tsx`) |
| **Email receipt** | ✅ | ✅ SMTP only (Phase 3b, `src/lib/email.ts`) — resend/sendgrid ยังไม่ implement |
| **`KIOSK_SERVICES` toggle** | ✅ | ✅ (`src/lib/site-config.ts`, menu + route guard) |
| **PostgreSQL transaction_log** | ✅ | ❌ dep ลงไว้แต่ไม่ได้ใช้ |
| **SIP2 retry logic** | ✅ | ❌ ENV มี แต่ `withSession` ไม่ retry |
| **`/api/config/logo` + `/welcome-video`** | (UI เรียก) | ✅ มีแล้ว (`app/api/config/`) |
