...
BOOL Cdemo_m24rl64Dlg::OnInitDialog()
{
	...
	for(DWORD i=0;i<AntennaIFCountOfReader;i++) {
		CString s1;
...
void Cdemo_m24rl64Dlg::OnTcnSelchangeTab1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if(idx == 1) {
		...
		for(INT32 i=0;i<m_taglist.GetCount();i++) {
			if(m_taglist[i].m_type == RFID_ISO15693_PICC_TI_HFI_PLUS_ID &&
				m_taglist[i].m_AIP == RFID_APL_ISO15693_ID) {
					...
					((CDlgTagTihfiPlus *)(m_tabDlgs[idx]))->m_cbbUIDs.SetItemData(((CDlgTagTihfiPlus *)(m_tabDlgs[idx]))->m_cbbUIDs.GetCount() -1,m_taglist[i].m_type) ;
			}
		}			
	}
	else if(idx == 4) {
		...
		for(INT32 i=0;i<m_taglist.GetCount();i++) {
			if(m_taglist[i].m_type == RFID_ISO15693_PICC_TIHFI_STANDARD_ID &&
				m_taglist[i].m_AIP == RFID_APL_ISO15693_ID) {
					...
					((CDlgTagTiHFIStandard *)(m_tabDlgs[idx]))->m_cbbUIDs.SetItemData(((CDlgTagTiHFIStandard *)(m_tabDlgs[idx]))->m_cbbUIDs.GetCount() -1,m_taglist[i].m_type) ;
			}
		}		
	}
	else if(idx == 5) {
		...
		for(INT32 i=0;i<m_taglist.GetCount();i++) {
			if((m_taglist[i].m_type ==RFID_ISO15693_PICC_ST_M24LR04E_ID || m_taglist[i].m_type ==RFID_ISO15693_PICC_ST_M24LR16E_ID   )   &&
				(m_taglist[i].m_AIP == RFID_APL_ISO15693_ID)) {
					((DlgTagM24LR16E *)(m_tabDlgs[idx]))->m_cbbUIDs.AddString(m_taglist[i].m_uid) ;
					((DlgTagM24LR16E *)(m_tabDlgs[idx]))->m_cbbUIDs.SetItemData(((DlgTagM24LR16E *)(m_tabDlgs[idx]))->m_cbbUIDs.GetCount() -1,m_taglist[i].m_type) ;
			}
		}		
	}
	else if(idx == 6) {
		...
		for(INT32 i=0;i<m_taglist.GetCount();i++) {
			if(m_taglist[i].m_type == RFID_ISO15693_PICC_ST_M24LR64_ID  &&
				m_taglist[i].m_AIP == RFID_APL_ISO15693_ID) {
					...
					((CDlgTagM24 *)(m_tabDlgs[idx]))->m_cbbUIDs.SetItemData(((CDlgTagM24 *)(m_tabDlgs[idx]))->m_cbbUIDs.GetCount() -1,m_taglist[i].m_type) ;
			}
		}		
	}
	else if (idx==7)
		...
		for(INT32 i=0;i<m_taglist.GetCount();i++) {
			if(m_taglist[i].m_type == RFID_ISO15693_PICC_RF430FR12XH_ID &&
				m_taglist[i].m_AIP == RFID_APL_ISO15693_ID) {
					...
					((CDlgTagRF430FRL15x *)(m_tabDlgs[idx]))->m_cbbUIDs.SetItemData(((CDlgTagRF430FRL15x *)(m_tabDlgs[idx]))->m_cbbUIDs.GetCount() -1,m_taglist[i].m_type) ;
			}
		}	
	}
	else if(idx ==8) {
		...
		for(INT32 i=0;i<m_taglist.GetCount();i++) {
			if(m_taglist[i].m_type == RFID_ISO14443A_PICC_NXP_MIFARE_S50_ID &&
				m_taglist[i].m_AIP == RFID_APL_ISO14443A_ID) {
					...
					((CDlgTagMFCLS50 *)(m_tabDlgs[idx]))->m_cbbUIDs.SetItemData(((CDlgTagMFCLS50 *)(m_tabDlgs[idx]))->m_cbbUIDs.GetCount() -1,m_taglist[i].m_type) ;
			}
		}		
	}
