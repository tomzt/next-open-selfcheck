...
// BorrowScreen — batch scan → review → confirm → SIP2 checkout (docs/requirements.md §5.1)

import { useTranslations } from 'next-intl'
import { useLocale } from 'next-intl'
import { useRouter } from 'next/navigation'
import { useEffect, useRef, useState } from 'react'
import { BookOpen, ScanLine, CheckCircle2, XCircle, ArrowLeft, X, Loader2, Mail } from 'lucide-react'
...
export default function BorrowScreen() {
  ...
  const runBatch = async () => {
    setPhase('processing')
    setResults(items.map((itemBarcode) => ({ itemBarcode, status: 'pending', title: null, dueDate: null })))
    ...
    for (const itemBarcode of items) {
      try {
        ...
        settled.push({ itemBarcode, status: 'error', title: null, dueDate: null })
      }
      setResults([...settled])
    }
    ...
    if (succeeded.length > 0) {
      fetch('/api/receipt/send', {
        ...
        body: JSON.stringify({
          locale,
          borrowed: succeeded.map((r) => ({ itemBarcode: r.itemBarcode, title: r.title, dueDate: r.dueDate })),
          returned: [],
        }),
      })
        .then((res) => res.json())
